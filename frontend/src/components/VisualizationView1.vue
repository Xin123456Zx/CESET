      ...
      <div class="grid grid-cols-[44px_2fr_2fr_2fr] gap-y-3 w-full place-items-center">
        ...
        <template v-for="(item,index) in dataList" :key="index">
            ...
            <div class="h-[150px] w-full flex items-center justify-center">
              <Cube3d2 v-if="item.output" roi-clip sync-camera auto-live :size="150" :url="item.output"/>
              <span v-else class="hint" :class="item.error && 'text-red-400'">{{item.error ? 'render failed' : 'rendering volume…'}}</span>
            </div>
            <div class="h-[150px] w-full flex items-center justify-center">
              <Cube3dData v-if="item.data" roi-clip sync-camera auto-live :size="150" :url="item.data"/>
              <span v-else class="hint" :class="item.error && 'text-red-400'">{{item.error ? 'render failed' : 'rendering volume…'}}</span>
            </div>
            <div class="h-[150px] w-full flex items-center justify-center">
              <Cube3d v-if="item.model" roi-clip sync-camera auto-live :size="150" :url="item.model"/>
              <span v-else class="hint" :class="item.error && 'text-red-400'">{{item.error ? 'render failed' : 'rendering volume…'}}</span>
            </div>
        </template>
      </div>
